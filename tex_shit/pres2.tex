...
\begin{frame}{Основные определения}
	...
	\begin{block}{Отвержение гипотезы}
		Пусть $c \in \mathbb{R}$. Будем говорить, что тест $T$ с критическим значением $c$ отвергает $H_0$ при реализациях равных $\hat{X}$ и $\hat{Y}$, если $T(\hat{X},\hat{Y}) \geqslant c$.
	\end{block}
	
	\pause
	
	\begin{block}{Уровень значимости}
		Будем говорить, что тест $T$ с критическим значением $c$ имеет уровень значимости $\alpha$,\\ если $P($отвергнуть $H_0 | H_0$ верна$) \leqslant \alpha$.
	\end{block}
...
\begin{frame}{Обозначения}
	...
	\begin{align*}
		J_1 &= \int_{\mathbb{R}^2} g(x-y)f(x)f(y)dxdy = \mathbb{E}[g(\xi - \eta)]\\
		\pause
		J_2 &= \int_{\mathbb{R}^2} g^2(x-y)f(x)f(y)dxdy = \mathbb{E}[g^2(\xi - \eta)] \\
		\pause
		J_3 &= \int_{\mathbb{R}^3} g(x-y)g(y-z)f(x)f(y)f(z)dxdydz = \\
		    &=\mathbb{E}[g(\xi - \eta)g(\eta - \gamma)] \\[0.2in]
		    \pause
...
\begin{frame}{О вычислении асимптотической мощности энергетического теста}
	\begin{block}{Теорема, Мелас 2024}
		...
		\[ 
		1 - F(z_{1-\frac{\alpha}{2}} - \frac{b}{a}) + F(-z_{1-\frac{\alpha}{2}} - \frac{b}{a})
		\]
		\pause
