...
Отчёт по научно-исследовательской работе (5 семестр)\\
...
\begin{flushright}
	\begin{minipage}{0.5\textwidth} % Ширина блока
		\raggedright
		\begin{spacing}{1.5} 
Выполнил:\\
...
В работе~\cite{melas2024} была получена формула для асимптотической мощности энергетического теста при неких ограничениях. Целью данной работы является эмпирическое исследование мощности энергетического критерия для проверки гипотезы о равенстве двух распределений в условиях близких альтернатив. Для достижения этой цели были поставлены следующие задачи:
\begin{itemize}
\item
исследовать скорость сходимости эмпирической мощности энергетического теста к теоретической при увеличении размера выборки для различных базовых распределений,
\item
...
\begin{aligned}
	...
	\Phi_{A} = \frac{1}{n^2} \sum_{1 \leq i \leq j \leq n} g(|X_i - X_j|), \quad
...
\noindent где $\Phi(t)$ --- функция стандартного нормального распределения, 
 $z_{1- \frac{\alpha}{2}}$ --- его $1- \frac{\alpha}{2}$ квантиль. А константы $a$, $b$ вычисляются, как было определено в~\eqref{eq:nums}. Всюду далее под статистикой для ET будет пониматься $n\Phi_{nn}$.
 
\newpage
 
\subsection{Тест Андерсона---Дарлинга для нескольких выборок (AD)}

Пусть $k \in \mathbb{N}$ --- количество выборок. Пусть $n_i$ --- размер $i$-й выборки, $ i \in 1, \ldots, k$. \\ Пусть $N = n_1 + \ldots + n_k$. Обозначим за $M_{ij}$ количество 
...
\newpage
\section{Результаты}
При вычислении значений в таблицах~\ref{tbl:ap_normal_h1_diff}--\ref{tbl:cauchy_pow_vs_h2} и на
рисунках~\ref{fig:normal_pow_vs_h2}--\ref{fig:cauchy_pow_vs_n_h1_diff}  для энергетического теста используется функция $g(x) = \ln(1+x^2)$.
...
Далее, в таблицах \ref{tbl:normal_pow_vs_h1}--\ref{tbl:cauchy_pow_vs_h2} представлены эмпирические мощности 5 тестов. На рисунках \ref{fig:normal_pow_vs_h2}--\ref{fig:cauchy_pow_vs_h1} представлены зависимости эмпирических мощностей от различий по масштабу/сдвигу, и на рисунках \ref{fig:normal_pow_vs_n_h2_diff}--\ref{fig:cauchy_pow_vs_n_h1_diff} представлены зависимости эмпирических мощностей от размера выборки.\\
\newpage
...
\clearpage
Далее, в таблицах~\ref{tbl:et_comp_normal_h1}--\ref{tbl:et_comp_cauchy_h2} $\text{ET}_1$ обозначает энергетический 
...
Как нетрудно заметить, при значениях параметров, соответствующих таблицам~\ref{tbl:ap_normal_h1_diff} и~\ref{tbl:ap_cauchy_h2_diff}, эмпирические мощности довольно быстро сходятся к асимптотическим. Но, например, в таблице~\ref{tbl:ap_cauchy_h1_diff} видно, что сходимость уже не такая быстрая при $h_1 \geq 4$.
